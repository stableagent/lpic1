# 103.5. ایجاد، مانیتور و متوقف کردن (kill) پردازش‌ها (processes)

**وزن:** 4

**توضیحات:** داوطلبان باید قادر به انجام مدیریت پایه پردازش‌ها باشند.

**حوزه‌های کلیدی دانش:**

* اجرای کارها (jobs) در پیش‌زمینه (foreground) و پس‌زمینه (background)
* ارسال سیگنال به یک برنامه برای ادامه اجرا پس از خروج از سیستم (logout)
* مانیتور کردن پردازش‌های فعال
* انتخاب و مرتب‌سازی پردازش‌ها برای نمایش
* ارسال سیگنال‌ها به پردازش‌ها
...
در این بخش درباره کارها (jobs) و پردازش‌ها (processes) صحبت می‌کنیم. آنچه باید بدانید این است که یک job دقیقاً چیست؟

اگر دستوری را از شل اجرا کنید، آن یک job است. 
...
یک job ممکن است اجرا شده و بلافاصله خارج شود و فقط برای مدت کوتاهی اجرا شود، اما برخی از jobها برای مدت طولانی اجرا می‌شوند.
...
این دستور برای 3600 ثانیه به خواب می‌رود، یعنی یک ساعت! بنابراین می‌توانید یک ساعت منتظر بمانید (با این فرض که نمی‌توانید از ترمینال خود استفاده کنید) یا می‌توانید یک job را در پس‌زمینه (background) اجرا کنید.

### کارهای پیش‌زمینه و پس‌زمینه (&) (Foreground and Background jobs)

یک **job پردازشی است که شل آن را مدیریت می‌کند**. به هر job یک شناسه (ID) ترتیبی اختصاص داده می‌شود. چون یک job یک پردازش است، هر job دارای یک PID مرتبط است. دو نوع وضعیت برای کارها وجود دارد:

1\. **Foreground (پیش‌زمینه)**: وقتی دستوری را در پنجره ترمینال وارد می‌کنیم، آن دستور پنجره ترمینال را تا زمان تکمیل اشغال می‌کند. این یک foreground job است.\
 2\. **Background (پس‌زمینه)**: وقتی علامت آمپرسند (**&**) را در انتهای یک خط فرمان وارد می‌کنیم، دستور بدون اشغال پنجره ترمینال اجرا می‌شود. اعلان شل بلافاصله پس از فشردن Return نمایش داده می‌شود. این نمونه‌ای از یک background job است.
...
> به طور پیش‌فرض، با فشردن enter از اتمام کارهای خاتمه یافته مطلع می‌شوید. دستور `set -b` را امتحان کنید تا بلافاصله مطلع شوید.

**Job Control Commands**

دستورات کنترل کار به **ما** اجازه می‌دهند کارها را در پیش‌زمینه یا پس‌زمینه قرار دهیم و کارها را شروع یا متوقف کنیم. جدول دستورات کنترل کار را توصیف می‌کند.
...
> اگر شناسه کاری (job id) ذکر نشود، bg و fg آخرین کار را در نظر می‌گیرند.
...
کارها **تنها در صورتی** کشته می‌شوند که گزینه `huponexit` تنظیم شده باشد! 
...
### ارسال سیگنال به یک برنامه برای ادامه اجرا پس از logout (Signal a program to continue running after logout)

### nohup

`Nohup` مخفف **no hangup** است و به این معنی است که حتی اگر شل والد قطع شود، کار همچنان ادامه می‌یابد. خروجی دستور **nohup** در فایل **nohup.out** نوشته می‌شود اگر نام فایلی برای تغییر مسیر در دستور ذکر نشده باشد.
...
حالا حتی اگر ترمینال را ببندیم و دوباره باز کنیم، ping همچنان در حال اجراست.
...
> توجه داشته باشید که دیگر نمی‌توانیم از دستورات `fg` و `bg` روی آن کار خاص استفاده کنیم.
...
ما همچنین می‌توانیم از دستور disown استفاده کنیم، این دستور پس از اینکه پردازشی لانچ شد و در پس‌زمینه قرار گرفت استفاده می‌شود؛ کار آن حذف یک کار (shell job) از لیست کارهای فعال شل است، بنابراین دیگر نمی‌توانیم از دستورات fg و bg برای آن کار خاص استفاده کنیم اما آن پردازش حتی پس از بستن شل فعلی به اجرای خود ادامه می‌دهد.
...
دستور **screen** در لینوکس قابلیت راه‌اندازی و استفاده از چندین جلسه (session) شل را از یک جلسه _ssh_ واحد فراهم می‌کند. 
...
1. **total **: displays the total installed memory _(MemTotal and SwapTotal i.epresent in /proc/meminfo)._
