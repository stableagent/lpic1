# 102.5 استفاده از مدیریت پکیج RPM و YUM

## **102.5 استفاده از مدیریت پکیج RPM و YUM**

**وزن:** 3

**توضیحات:** داوطلبان باید قادر به انجام مدیریت پکیج با استفاده از ابزارهای RPM و YUM باشند.

**حوزه‌های کلیدی دانش:**

* نصب، نصب مجدد، ارتقا و حذف پکیج‌ها با استفاده از RPM و YUM
* به دست آوردن اطلاعات در مورد پکیج‌های RPM مانند نسخه، وضعیت، وابستگی‌ها، یکپارچگی و امضاها
* تعیین اینکه یک پکیج چه فایل‌هایی را ارائه می‌دهد و همچنین یافتن اینکه یک فایل خاص از کدام پکیج می‌آید
...
در این درس یاد می‌گیریم که چگونه با پکیج‌ها در توزیع‌های RedHat کار کنیم. اگر با مفاهیم پکیج و مدیریت پکیج در لینوکس آشنا نیستید، لطفاً بخش اول درس قبل را مطالعه کنید.

![](.gitbook/assets/redpack-red.jpg)

## rpm

RPM (Red Hat Package Manager) یک مدیریت پکیج قدرتمند است که می‌تواند برای ساخت، نصب، کوئری (query)، تایید، به‌روزرسانی و پاک کردن پکیج‌های نرم‌افزاری منفرد استفاده شود.

یک پکیج شامل آرشیوی از فایل‌ها و متا-دیتا (meta-data) است که برای نصب و پاک کردن فایل‌های آرشیو استفاده می‌شود. متا-دیتا شامل اسکریپت‌های کمکی، ویژگی‌های فایل و اطلاعات توصیفی در مورد پکیج است.

پکیج‌ها در دو نوع عرضه می‌شوند: **پکیج‌های باینری**، که برای کپسوله‌سازی نرم‌افزار جهت نصب استفاده می‌شوند، و **پکیج‌های منبع (source)**، که حاوی کد منبع و دستورالعمل‌های لازم برای تولید پکیج‌های باینری هستند. RPM تنها راه برای نصب پکیج‌ها در سیستم‌های لینوکس است؛ اگر پکیج‌ها را با استفاده از کد منبع نصب کرده‌اید، rpm آن‌ها را مدیریت نخواهد کرد.

![](.gitbook/assets/redpack-rpmbox.jpg)

RPM با فایل‌های .rpm سر و کار دارد که حاوی اطلاعات واقعی در مورد پکیج‌ها هستند، مانند: چیست، از کجا می‌آید، اطلاعات وابستگی‌ها، اطلاعات نسخه و غیره.

### کجا می‌توان پکیج‌های RPM را پیدا کرد

لیستی از سایت‌های rpm که در آن‌ها می‌توانیم تمام پکیج‌های RPM را پیدا و دانلود کنیم:
...
آن‌ها همگی فایل‌های باینری هستند، سعی کنید `cat Basenames` را ببینید.

پنج حالت اصلی برای دستور RPM وجود دارد:
